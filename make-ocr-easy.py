...
# def search(region,compressed,drawable):
#     total = len(compressed)
#     pdb.gimp_message(total)
#     marked = [[False for i in range(region.y,region.y+region.h)] for j in range(region.x,region.x+region.w)]
#     limit = {'xMin': region.x, 'yMin': region.y, 'xMax': region.x+region.w-1, 'yMax': region.y+region.h-1}
#     convexList = []
#     count = 1
...
#             pdb.gimp_message("Progress: %d%%"%round((float(count)/total)*100))
#         if marked[i-region.x][j-region.y]:
#             continue            
#         if region[i,j] != '\x00':
#             convex,marked,count = BFS(region,i,j,marked,limit,drawable,count,total)
#             convexList.append(convex)
#         else:
#             marked[i-region.x][j-region.y] = True
#             count+=1
...
#     non_empty, x1, y1, x2, y2 = pdb.gimp_selection_bounds(image)
#     region = selection.get_pixel_rgn(x1,y1,x2-x1,y2-y1)
#     compressed = [(x,y) for x in range(x1,x2) for y in range(y1,y2) if region[x,y] != '\x00']
#     convexList = search(region,compressed,drawable)
...
# pdb.gimp_image_select_item(image, 1, channel)
# image.add_layer(pdb.gimp_layer_copy(base, True),0)
# pdb.file_png_save_defaults(image, drawable, os.path.join(path,name), os.path.join(path,name))
